...
import { getRecentProjectReleases, releaseMatchesProject } from "./releaseScope";
...
const recent = getRecentProjectReleases(releases, "LHPM", 5);
...
assertEqual(releaseMatchesProject(releases[2], "LHPM"), true, "project key matching is case-insensitive");
assertEqual(getRecentProjectReleases(releases, null, 5).length, 0, "missing project key does not fall back to global releases");
