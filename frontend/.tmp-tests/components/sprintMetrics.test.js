...
function sprintMetricsResponse(hasStoryPoints) {
    return {
        ...
        unavailable_reason: hasStoryPoints ? "No Jira changelog history is available for this scope." : sprintMetrics_1.sprintNoStoryPointsReason,
        metrics,
        metric_issue_keys: {
            open_blockers: [],
        ...
        delivery_confidence_explanations: hasStoryPoints ? [] : [sprintMetrics_1.sprintNoStoryPointsReason],
...
function workloadDistribution(overrides = {}) {
...
assertEqual((0, sprintMetrics_1.classifyWorkDistribution)("healthy"), "good", "stored healthy band maps to good");
assertEqual((0, sprintMetrics_1.classifyWorkDistribution)("watch"), "warning", "stored watch band maps to warning");
assertEqual((0, sprintMetrics_1.classifyWorkDistribution)("critical"), "critical", "stored critical band maps to critical");
...
assertEqual((0, sprintMetrics_1.getSprintStoryPointUnavailableReason)(sprintMetricsResponse(false)), sprintMetrics_1.sprintNoStoryPointsReason, "story-point unavailable reason comes from metric availability");
assertEqual((0, sprintMetrics_1.getSprintMetricUnavailableBadge)(sprintMetrics_1.sprintNoStoryPointsReason), "No story points", "story-point unavailable reason maps to a muted badge");
...
const prerequisiteInconclusiveResponse = {
...
const prerequisiteInconclusiveUi = (0, sprintMetrics_1.buildSprintStoryPointUiVisibility)(prerequisiteInconclusiveResponse);
assertEqual((0, sprintMetrics_1.getSprintStoryPointCoverageStatus)(prerequisiteInconclusiveResponse), "COMPUTED", "complete point coverage remains computed when confidence prerequisites are missing");
assertEqual((0, sprintMetrics_1.hasSprintStoryPoints)(prerequisiteInconclusiveResponse), true, "confidence prerequisites do not hide independently available point features");
assertEqual((0, sprintMetrics_1.hasSprintDeliveryConfidence)(prerequisiteInconclusiveResponse), false, "inconclusive prerequisites withhold current delivery confidence");
assertEqual(prerequisiteInconclusiveUi.showPointValues, false, "inconclusive confidence does not render score component inputs");
assertEqual(prerequisiteInconclusiveUi.showDeliveryConfidenceBreakdown, false, "inconclusive confidence does not render a component breakdown");
assertEqual(prerequisiteInconclusiveUi.showTeamPredictability, true, "complete point coverage preserves independent predictability features");
assertEqual(prerequisiteInconclusiveUi.showCommitmentReliability, true, "complete point coverage preserves commitment reliability");
assertEqual(prerequisiteInconclusiveUi.showDeliveryConfidenceTrend, true, "historical confidence remains eligible to render with a null current point");
assertEqual(prerequisiteInconclusiveUi.showStoryPointChartEmptyState, false, "missing confidence prerequisites are not mislabeled as missing story points");
const partialCoverageInconclusiveResponse = {
...
assertDeepEqual((0, sprintMetrics_1.getSprintMetricExplanations)({
    ...sprintMetricsResponse(true),
    metric_availability: {
        ...sprintMetricsResponse(true).metric_availability,
        metrics: {
...
}, "reopen_rate_pct"), [repeatedReopenExplanation], "sprint display exposes repeated-reopen evidence");
assertEqual((0, sprintMetrics_1.getMetricStatus)("reopen_rate_pct", 200), "critical", "reopen values above 100 remain valid risk inputs");
const emptyScopeReason = "No tickets are available for this scope.";
const emptyScopeResponse = {
    ...sprintMetricsResponse(true),
    metrics: {
        ...metrics,
...
const emptyCurrentScopeDisplay = (0, sprintMetrics_1.getSprintMetricDisplay)(emptyScopeResponse, "committed_scope");
...
const partialCompletionResponse = {
    ...sprintMetricsResponse(true),
    metrics: {
        ...metrics,
        committed_scope: 2,
...
const partialCompletionDisplay = (0, sprintMetrics_1.getSprintMetricDisplay)(partialCompletionResponse, "completed_scope_pct");
...
const workDistribution = (0, sprintMetrics_1.buildWorkDistributionDisplayModel)(workloadDistribution());
...
assertDeepEqual(workDistribution.details, [
    "Risk band: critical",
...
const workloadAt35 = (0, sprintMetrics_1.buildWorkDistributionDisplayModel)(workloadDistribution({ percentage: 35, evidence: { workload_concentration_pct: 35, risk_band: "watch" } }));
const workloadAt50 = (0, sprintMetrics_1.buildWorkDistributionDisplayModel)(workloadDistribution({ percentage: 50, evidence: { workload_concentration_pct: 50, risk_band: "watch" } }));
const workloadAbove50 = (0, sprintMetrics_1.buildWorkDistributionDisplayModel)(workloadDistribution({ percentage: 50.01, evidence: { workload_concentration_pct: 50.01, risk_band: "critical" } }));
...
const partialPointWorkDistribution = (0, sprintMetrics_1.buildWorkDistributionDisplayModel)(workloadDistribution({
    status: "PARTIAL",
    percentage: 75,
    explanations: [
...
const inconclusiveWorkDistribution = (0, sprintMetrics_1.buildWorkDistributionDisplayModel)(workloadDistribution({
    status: "INCONCLUSIVE",
    percentage: null,
...
assertEqual(inconclusiveWorkDistribution.comparison, "Workload distribution requires at least 50% story-point coverage.", "work distribution displays the authoritative inconclusive explanation");
assertEqual(inconclusiveWorkDistribution.badge, "Inconclusive", "inconclusive workload is labeled");
assertDeepEqual(inconclusiveWorkDistribution.details, ["Tickets missing status: LHPM-4"], "inconclusive workload displays missing-status evidence");
const notApplicableWorkDistribution = (0, sprintMetrics_1.buildWorkDistributionDisplayModel)(workloadDistribution({
...
const zeroPointWorkDistribution = (0, sprintMetrics_1.buildWorkDistributionDisplayModel)(workloadDistribution({
    status: "NOT_COMPUTED",
    percentage: null,
...
assertEqual(zeroPointWorkDistribution.comparison, "Included active story points sum to zero.", "zero-point workload displays the stored reason");
const baseWorkStateResponse = {
    ...sprintMetricsResponse(true),
    metrics: { ...metrics },
    metric_availability: {
        ...sprintMetricsResponse(true).metric_availability,
...
const workState = (0, sprintMetrics_1.buildSprintWorkStateDisplayModel)(baseWorkStateResponse, [
    { status: "Done" },
    { status: "Closed" },
    { status: "In Progress" },
]);
assertDeepEqual(workState.details, [
    "Current scope: 19",
    "In progress: 7",
    "Not started: 6",
    "Done: 2",
...
const activeUnfinishedDisplay = (0, sprintMetrics_1.getSprintMetricDisplay)(activeWorkStateResponse, "rollover_count");
...
const activeWorkState = (0, sprintMetrics_1.buildSprintWorkStateDisplayModel)(activeWorkStateResponse, [{ status: "Done" }]);
assertEqual(activeWorkState.details.includes("Unfinished closed-sprint scope: N/A (not applicable)"), true, "active work state does not present unavailable unfinished scope as zero");
const closedWorkStateResponse = {
    ...baseWorkStateResponse,
    metrics: { ...baseWorkStateResponse.metrics, rollover_count: 2 },
};
const closedWorkState = (0, sprintMetrics_1.buildSprintWorkStateDisplayModel)(closedWorkStateResponse, [{ status: "Done" }]);
assertEqual(closedWorkState.details.includes("Unfinished closed-sprint scope: 2"), true, "closed work state displays the computed unfinished count");
const emptyWorkStateResponse = {
    ...baseWorkStateResponse,
    metrics: {
...
const emptyWorkState = (0, sprintMetrics_1.buildSprintWorkStateDisplayModel)(emptyWorkStateResponse, []);
...
const workStatePartialReason = "In-progress count is partial because 1 current sprint ticket(s) have no status. The returned value is a confirmed minimum.";
const partialWorkStateResponse = {
    ...baseWorkStateResponse,
    metrics: {
        ...baseWorkStateResponse.metrics,
        committed_scope: 3,
...
const partialWorkState = (0, sprintMetrics_1.buildSprintWorkStateDisplayModel)(partialWorkStateResponse, [
    { status: "Done" },
    { status: "In Progress" },
    { status: null },
]);
...
assertEqual(partialWorkState.details.includes("Missing status: LHPM-3"), true, "partial work state exposes missing-status keys");
const partialInProgressDisplay = (0, sprintMetrics_1.getSprintMetricDisplay)(partialWorkStateResponse, "in_progress_count");
assertEqual(partialInProgressDisplay.badge, "Partial", "partial count card displays its partial badge");
assertDeepEqual(partialInProgressDisplay.missingIssueKeys, ["LHPM-3"], "partial count card exposes missing-status keys");
